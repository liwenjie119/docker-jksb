...
CMD env > /root/env && sed -i '/GPG_KEYS/d' /root/env && sed -i '/no_proxy/d' /root/env && sed -i -e 's/^/export /' /root/env && cat /root/env >> /etc/profile && chmod +x /root/env &&bash /root/start.sh
